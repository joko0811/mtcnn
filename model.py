...
class MTCNN(nn.Module):
    ...
    def forward(self, img, save_path=None, return_img=False):
    ...
    def detect(self, img, landmarks=False):
        ...
        for box, point in zip(batch_boxes, batch_points):
            if len(box) == 0:
                boxes.append(torch.tensor([]).to(device=self.device))
                probs.append(torch.tensor([]).to(device=self.device))
                points.append(torch.tensor([]).to(device=self.device))
            elif self.select_largest:
                box_order = torch.argsort(
                    (box[:, 2] - box[:, 0]) * (box[:, 3] - box[:, 1]), descending=True
                )
                box = box[box_order]
