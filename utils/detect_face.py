...
def detect_face(imgs, minsize, pnet, rnet, onet, threshold, factor, device):
    ...
    if isinstance(imgs, (np.ndarray, torch.Tensor)):
        if isinstance(imgs, np.ndarray):
            ...
            imgs = imgs.permute(2, 0, 1).type(model_dtype)
    ...
    for b_i in range(batch_size):
        b_i_inds = torch.eq(image_inds, b_i)
...
def nms_torch(boxes, scores, threshold, method):
    if boxes.numel() == 0:
    ...
    while I.numel() > 0:
        ...
        # max(xx2 - xx1 + 1, 0)
        w = torch.where((xx2 - xx1 + 1) < 0, 0, xx2 - xx1 + 1)
        h = torch.where((yy2 - yy1 + 1) < 0, 0, yy2 - yy1 + 1)
...
def batched_nms_numpy(boxes, scores, idxs, threshold, method):
    ...
    keep = nms_torch(boxes_for_nms, scores, threshold, method)
    return keep.to(dtype=torch.int)
